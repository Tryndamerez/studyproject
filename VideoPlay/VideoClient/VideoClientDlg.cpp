...
BOOL CVideoClientDlg::OnInitDialog()
{
	...
	//m_url.SetWindowText(_T("file:///D:\\yuankong\\studyproject\\VideoPlay\\VideoClient\\股市讨论.mp4"));
...
void CVideoClientDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	if (nSBCode == 5) {
		...
		GetDlgItemText(IDC_STATIC_TIME, strPosition);
...
{//音量
	...
	if (nSBCode == 3)
	{
		//TRACE("pos %p volume %p cur %p pos %d %d \r\n", &m_pos, &m_volume, pScrollBar, nPos,nSBCode);
		CString strVolume;
		strVolume.Format(_T("%d%%"), 100 - nPos);
		GetDlgItemText(IDC_STATIC_VOLUME, strVolume);
