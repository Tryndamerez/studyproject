...
public:
	...
	bool IsValid()  //返回true表示有效 返回false表示线程异常或者被终止
	{
...
private:
	HANDLE m_hThread;
	bool m_bStatus;//false 表示线程将要结束 true 表示线程正在运行
...
public:
	...
	int DispatchWorkwer(const ThreadWorker& worker)//返回-1 表示分配失败 大于等于0表示分配第n个线程做这件事
	{
		int index = -1;
		m_lock.lock();
		for (size_t i = 0; i < m_threads.size(); i++)
