...
int AcceptOverlapped<op>::AcceptWorker()
{
	...
	if (m_client->GetBufferSize() > 0) {
		...
		int ret = WSARecv((SOCKET)*m_client, m_client->RecvWSABuffer(), 1, *m_client, 
			&m_client->flags(), m_client->RecvOverlapped(), NULL);
...
EdoyunClient::EdoyunClient() :m_isbusy(false), m_flags(0),
...
bool EdoyunServer::StartService() {
	...
	m_pool.Invoke();
	m_pool.DispatchWorkwer(ThreadWorker(this, (FUNCTYPE)&EdoyunServer::threadIocp));
...
int EdoyunServer::threadIocp() {
	...
	if (GetQueuedCompletionStatus(m_hIOCP, &tranferred, 
		...
		if (CompletionKey != 0) {
			...
			case EAccept:
			{
				...
				m_pool.DispatchWorkwer(pOver->m_worker);
			}
			break;
			case ERecv:
			{
